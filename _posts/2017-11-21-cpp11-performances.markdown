...
Modern architecture generally several levels of cache between the CPU and RAM.
...
top notch algorithms to optimize waht as to be put in so that the cores do not
...
a cache hit. If not, it is a cache miss and the CPU will try to find the memory
in L2 cache. The process continues until the data is found which can end up in
the RAM (the slowest location).

The number of CPU cycle to access a piece of data in memory give a clear
...
The `average` function loop on all the structures contained in the array. This
...
the MMX registers. As a matter of fact the number of operation to sum all the
...
There is not call to the `square(int)` function anymore: this is called
inlining.

The C++ compilers strive to inline the maximum of function to improve the
...
Even, static would not help because there could be some subclasses of
...
This makes a big difference because if we tell the compiler to trust us on the
real types contained in the array, he will be smart enought to avoid using the
vtable indirection.
...
The value `4200000` is directly passed in `%esi` to be displayed !
...
last C++ standard version leads to higher performance... for free ! Morever,
...
contiguous memory containing the objects, there is no need for reallocation as
it is the case with `std::vector`. Nevertheless, if you know the number of
elements that will be stored, you can use the `std::vector<>::reserve` method
and in this case the `std::vector` is the best. Conversely, due to the tree
structure of the `std::set` container and the fact the objects are always
ordered, this is the worse container.
